angular.module('iotapp', ['ionic', 'chart.js'])
  ...
  .factory("globals", function ($ionicLoading) {
   ...
   //globalsService.rooturl = "http://localhost:3000";
    globalsService.roourl = "http://iotindustry40.mybluemix.net";
  ...
  */
  ...
  .controller("DashCtrl", function ($timeout, $scope, dbs, ChartService, globals, backend, $ionicLoading) {
    ...
    $scope.gauges = [];
    var gaugelabels = ["F1 Motor Torque on the no load",
      "F1 Mill Motor Torque on the no load",
      "F1 Mill Motor Torque on the load",
      "F1 Main Drive Speed ripple",
      "F1 Motor Stator Temperature difference",
      "F1 Cylinder Position Difference",
      "F1 Roll Force difference",
      "F1 Main Drive Speed diff(control-actual)"
    ];
    ...
    $scope.init = function () {
      ...
      backend.get("/simulation/getsimulstatus", function (data) {
        $timeout(function () {
    ...
    $scope.setDash = function (idx) {
      ...
      return;
      if (idx == 1) {
        backend.get("/dashboard/sensormeasurement", function (data) {
          ...
          data.forEach(function (item, idx) {
            ...
            if (sensidx<3){
            console.log(sensidx);
            if ($scope.sensormeasurement[sensidx].data.length==10) $scope.sensormeasurement[sensidx].data.shift();
            $scope.sensormeasurement[sensidx].data.push(parseInt(value, 10));
            ...
            $scope.sensormeasurement[sensidx].series = ["v"];
            }
    ...
    $scope.$on("iotbuffer", function (event, args) {
      ...
      //console.log("iotbuffer received from socket server, eventtype " + evtype, args);
      ...
      if (evtype == "sensormeasurement.json") {
        ...
        if ($scope.sensormeasurement[sensidx].data.length == 10) $scope.sensormeasurement[sensidx].data.shift();
        $scope.sensormeasurement[sensidx].data.push(parseInt(ev.Value, 10));
        if ($scope.sensormeasurement[sensidx].labels.length == 10) $scope.sensormeasurement[sensidx].labels.shift();
        $scope.sensormeasurement[sensidx].labels.push("|");
