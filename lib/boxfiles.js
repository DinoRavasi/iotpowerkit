...
function loadBoxFiles(callback){
    
    dbs.listByField("config","name","boxfiles",function(data){
        //console.log(data.rows[0].doc);
       

        boxfiles=data.rows[0].doc.data;
        var bcount=0;
        for (var k in boxfiles){
            bcount++;
        }
        console.log(bcount+" boxfiles loaded");
        if (callback) callback(boxfiles);
...
exports.loadBoxFiles=loadBoxFiles;