...
function getOperationShutdownTime(options, callback) {
    ...
    if (tfdate.trim() == "") {
        tfdate = moment("23-10-2016", "DD-MM-YYYY");
    } else {
    ...
    fdbs.readJsonFileRemoteHttps(fname, function (data) { //OEE2.csv
        ...
        data.rows.forEach(function (item, idx) {
            ...
            var datarecord = utils.getSafeField(item, "Anno-mese-gg");
            ...
            var itemlinea = utils.getSafeField(item, "LINEA");
            ...
            if (doIt) {
                ...
                if (issame) {
                    ...
                    var true_duration = utils.getSafeField(item, "Duratatotale");
...
/*
      ...
      /*  fdbs.readJsonFileRemoteHttps("/shared/static/icv1y2y486rnh2c3tvkcgpfktdoca7xp.json", function (makerdata) {
            //fdbs.readJsonFile("data/MAKER_Scarti_sigarette.json", function (makerdata) {
            var totmaker = 0;
            console.log("maker rows: ", makerdata.rows.length);




            makerdata.rows.forEach(function (item, idx) {
                //console.log(item);
                var truestart = item.field1;
                var truestartday = truestart.split(" ")[0];
                var tsday = moment(truestartday, "MM-DD-YYYY").toDate();
                ...
                if (issame) {
                    var paker1 = parseFloat(item.Maker_82);
                    totmaker += paker1;
                }
            });

            console.log("totpaker", totpaker, "totmaker", totmaker);
        ...
        var newmachines=[];
        fmodel.machines.forEach(function(item,idx){
            var mname=item.name;
